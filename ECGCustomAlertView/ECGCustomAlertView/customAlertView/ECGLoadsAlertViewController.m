...
//    self.alertView.center = CGPointMake(ECGScreenWidth / 2, ECGScreenHeight / 2);
//}
//
////竖屏
//- (void)changeSubView {
...
- (void)dismissKB {
    ...
    for (id obj in subviews) {
        if ([obj isKindOfClass:[ECGCustomAlertView class]]) {
            ECGCustomAlertView *customView = obj;
            for (id objInput in customView.subviews) {
                NSLog(@"%@",objInput);
                ...
                if ([objInput isKindOfClass:[UITextView class]]) {
                    UITextView *theTextView = objInput;
                    if ([objInput isFirstResponder]) {
        ...
        if ([obj isKindOfClass:[UITextField class]]) {
            UITextField *theTextField = obj;
            if ([obj isFirstResponder]) {
                [theTextField resignFirstResponder];
            }
        }
        if ([obj isKindOfClass:[UITextView class]]) {
            UITextView *theTextView = obj;
            if ([obj isFirstResponder]) {
                [theTextView resignFirstResponder];
            }
        }
