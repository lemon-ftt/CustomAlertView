...
- (void)updateLayout {
    ...
    _view.center = CGPointMake(ECGScreenWidth/2, ECGScreenHeight/2);
...
+ (void)showCustomContainsImageToast:(NSString *)text withImage:(UIImage *)image waitDone:(BOOL)bl duration:(uint64_t)duration {
    ...
    for (UIView *subView in subviews) {
        if ([subView isKindOfClass:[ECGCustomAlertView class]]) {
 ...
 */
+ (void)showToast:(NSString *)text duration:(uint64_t)duration {
    [self dismissWaitPopViewCompletion:^{
 ...
 */
+ (void)dismissWaitPopViewCompletion:(void(^)(void))completion {
    [WaitPopView dismissWaitPopViewCompletion:^{
        if (completion) {
            completion();
        }
        NSLog(@"弹出窗被关闭了");
...
- (void)setup{
    ...
    if (count == 1) {
        ...
        //增加线条
        UIView *lineView=[[UIView alloc]initWithFrame:CGRectMake(ECGMargin, self.frame.size.height - ECGButtonHeight - ECGMargin, ECGAlertViewWidth - ECGMargin * 2, 0.5)];
        lineView.backgroundColor=[UIColor grayColor];
        [self addSubview:lineView];
        
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(ECGMargin, self.frame.size.height - ECGButtonHeight - ECGMargin-0.5, ECGAlertViewWidth - ECGMargin * 2, ECGButtonHeight)];
        ...
        for (int i = 0; i < 2; i++) {
            ...
            UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(ECGMargin + (ECGMargin + btnWidth) * i, self.frame.size.height - ECGButtonHeight - ECGMargin-0.5, btnWidth, ECGButtonHeight)];
